# Underlying data structures during the implementation, numpy array. Other natives of Python might also be used
import numpy as np

# Euclidean and Haversine distance method to calculate distance/distance matrix
...
class DeliveryEnvironment(object):
    def __init__(self, data, n_stops = 10, max_box = 10, method = "euclidean_distance", **kwargs):
    ...
    def _generate_q_values(self, box_size = 0.2):
        ...
        if self.method in ["euclidean_distance", "traffic_box"]:
            xy = np.column_stack([self.x, self.y])
            self.q_stops = cdist(xy, xy)
        elif self.method == 'haversine_distance':
            n = self.data.shape[0]
            co_list = []
            for i in self.data:
                for j in i:
                    co_list.append(radians(j))
            self.q_stops = haversine_distances(np.array(co_list).reshape(n, 2)) * 6371000/1000
    ...
    def _get_reward(self, state, new_state):
        ...
        if self.method in ["euclidean_distance", "haversine_distance"]:
            return base_reward
        elif self.method == "time":
